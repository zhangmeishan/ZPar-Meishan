...
protected:
   virtual void work(const CStringVector *sentence , const CSTackFeatsVec  & stackfeats, CTwoStringVector *retval, double *out_scores=NULL, unsigned long nBest=1, const CBitArray *prunes=NULL);

public:
   enum SCORE_UPDATE {eSubtract=0, eAdd};

   virtual bool train(const CStringVector *sentence , const CSTackFeatsVec  & stackfeats, const CTwoStringVector *correct);
   virtual void tag(const CStringVector *sentence , const CSTackFeatsVec  & stackfeats, CTwoStringVector *retval, double *out_scores=NULL, unsigned long nBest=1, const CBitArray *prunes=NULL);
...
public:
   ...
   inline void updateScoreForState(const CStringVector *sentence , const CSTackFeatsVec  & stackfeats, const tagger::CSubStateItem *item, const tagger::SCORE_TYPE &amount) {
      static unsigned tmp_i, tmp_j;
      for (tmp_i=0; tmp_i<item->size(); ++tmp_i) {
         getOrUpdateSeparateScore(sentence, stackfeats, item, tmp_i, amount, m_nTrainingRound);
         for (tmp_j=item->getWordStart(tmp_i)+1; tmp_j<item->getWordEnd(tmp_i)+1; ++tmp_j)
            getOrUpdateAppendScore(sentence, stackfeats, item, tmp_i, tmp_j, amount, m_nTrainingRound);
      }
   }
   tagger::SCORE_TYPE getOrUpdateSeparateScore(const CStringVector *tagged , const CSTackFeatsVec  & stackfeats, const tagger::CSubStateItem *item, unsigned long index, tagger::SCORE_TYPE amount=0, unsigned long round=0);
   tagger::SCORE_TYPE getOrUpdateAppendScore(const CStringVector *tagged , const CSTackFeatsVec  & stackfeats, const tagger::CSubStateItem *item, unsigned long index, unsigned long char_index, tagger::SCORE_TYPE amount=0, unsigned long round=0);
