...
void process(const std::string sInputFile, const std::string sOutputFile, const std::string sFeatureFile, const unsigned long nBest, const unsigned long nMaxSentSize
        , const bool bSegmented, bool bScores, const std::string &sStackingPath) {
   ...
   while( bReadSuccessful ) {
      ...
      if (bSegmented) {
         ...
         if ( unsegmented_sent->size() >= tagger::MAX_SENTENCE_SIZE ) {
            ...
            tagger.tag(unsegmented_sent, stackfeats, outout_sent, outout_scores, nBest, &prunes);
      }
      else {
         if ( input_sent->size() >= tagger::MAX_SENTENCE_SIZE ) {
            ...
            tagger.tag(input_sent, stackfeats, outout_sent, outout_scores, nBest);
      }
      //
      // Ouptut sent
      //
   ...
   if(!sStackingPath.empty()) isstack.close();
...
//std::cout << out.str() << std::endl;
      ...
      process(sInputFile, sToFile, options.args[1], nBest, nMaxSentSize, bSegmented, bScores, sStackingPath);
      return 0;
