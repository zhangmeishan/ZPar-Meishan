...
SCORE_TYPE getOrUpdateSeparateScore(CSegmentor *segmentor, const CStringVector* sentence, const CStateItem* item, SCORE_TYPE amount=0, int round=0){
   ...
   //std::cout << segmentor->stack_sent->size() << " " << sentence->size() << std::endl;
   //for(int idx = 0; idx < segmentor->stack_sent->size(); idx++)
   //{
   //    std::cout << segmentor->stack_sent->at(idx) << " ";
   //}
   //std::cout << std::endl;
   if(segmentor->stack_sent != NULL && segmentor->stack_sent->size() == sentence->size())
   {
       int previoustag = 0, curtag = 0, nexttag = 0;
       //if(index+1 > 0)
       {
           previoustag = segmentor->stack_sent->at(end).compare("B") ? 2: 1;
       }
       if(end+1 < sentence->size()-1)
       {
           nexttag = segmentor->stack_sent->at(end+2).compare("B") ? 2: 1;
       }
       if(end+1 <= sentence->size()-1)
       {
           curtag = segmentor->stack_sent->at(end+1).compare("B") ? 2: 1;
       }

       nReturn += weight.m_mapSepStackedPrevCurChar.getOrUpdateScore(std::make_pair(first_char_next_word, previoustag), which_score, amount, round);
       nReturn += weight.m_mapSepStackedCurCurChar.getOrUpdateScore(std::make_pair(first_char_next_word, curtag), which_score, amount, round);
       nReturn += weight.m_mapSepStackedNextCurChar.getOrUpdateScore(std::make_pair(first_char_next_word, nexttag), which_score, amount, round);
       nReturn += weight.m_mapSepStackedCurChar.getOrUpdateScore(curtag, which_score, amount, round);
       nReturn += weight.m_mapSepStackedPrevCurCurChar.getOrUpdateScore(std::make_pair(first_char_next_word, previoustag*10+curtag), which_score, amount, round);
       nReturn += weight.m_mapSepStackedNextCurCurChar.getOrUpdateScore(std::make_pair(first_char_next_word, nexttag*10+curtag), which_score, amount, round);
   }
...
SCORE_TYPE getOrUpdateAppendScore(CSegmentor *segmentor, const CStringVector* sentence, const CStateItem* item, int index, SCORE_TYPE amount=0, int round=0){
   ...
   if(segmentor->stack_sent != NULL && segmentor->stack_sent->size() == sentence->size())
   {
       ...
       //if(index+1 > 0)
       {
           previoustag = segmentor->stack_sent->at(index).compare("B") == 0 ? 2: 1;
       }
       if(index+1 < sentence->size()-1)
       {
           nexttag = segmentor->stack_sent->at(index+2).compare("B") == 0 ? 2: 1;
       }
       if(index+1 <= sentence->size()-1)
       {
           curtag = segmentor->stack_sent->at(index+1).compare("B") == 0 ? 2: 1;
       }

       retval += weight.m_mapAppStackedPrevCurChar.getOrUpdateScore(std::make_pair(current_char, previoustag), which_score, amount, round);
       retval += weight.m_mapAppStackedCurCurChar.getOrUpdateScore(std::make_pair(current_char, curtag), which_score, amount, round);
       retval += weight.m_mapAppStackedNextCurChar.getOrUpdateScore(std::make_pair(current_char, nexttag), which_score, amount, round);
       retval += weight.m_mapAppStackedCurChar.getOrUpdateScore(curtag, which_score, amount, round);
       retval += weight.m_mapAppStackedPrevCurCurChar.getOrUpdateScore(std::make_pair(current_char, previoustag*10+curtag), which_score, amount, round);
       retval += weight.m_mapAppStackedNextCurCurChar.getOrUpdateScore(std::make_pair(current_char, nexttag*10+curtag), which_score, amount, round);
   }
      ...
      for ( index=0; index<std::min(nBest, nBestGen); ++index ) {
         ...
         while (!temp->empty()) {
            for (temp_index=temp->getWordStart(); temp_index<=temp->getWordEnd(); ++temp_index) {
               //std::cout << temp->getWordStart() << " " << temp->getWordEnd() << " " << temp_index << std::endl;
