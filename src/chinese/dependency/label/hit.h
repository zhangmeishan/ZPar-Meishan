 ...
 *                                                              * 
 ****************************************************************/
const std::string PENN_HIT_STRINGS[] = {
   "-NONE-",
   "root",
   "locationfin",
   "locationini",
...
const unsigned long PENN_HIT_COUNT_BITS = 7;
...
public:
   ...
   enum {ROOT=1};
   enum {FIRST=1};
   enum {COUNT=PENN_HIT_COUNT};
   enum {MAX_COUNT=COUNT};
   enum {SIZE=PENN_HIT_COUNT_BITS};
...
public:
   ...
   void load(const std::string &str) { 
      m_code = PENN_HIT_NONE;
      for (int i=FIRST; i<COUNT; ++i) {
         if (PENN_HIT_STRINGS[i]==str) {
            m_code = i;
            return;
         }
      }
   ...
   const std::string &str() const { 
      return PENN_HIT_STRINGS[ m_code ];
   }
