...
SCORE_TYPE CTagger::getOrUpdateSeparateScore(const CStringVector *sentence, const CSubStateItem *item, unsigned long index, SCORE_TYPE amount,
    unsigned long round) {
  ...
  if (index > 0) {
    ...
    if (index > 1)
      nReturn += m_weights->m_mapLastWordByWord.getOrUpdateScore(word_2_word_1, m_nScoreIndex, amount, round);
...
//         nReturn += m_weights->m_mapLengthByTagAndLastChar.getOrUpdateScore( std::make_pair(last_char_1, (length_1<<CTag::SIZE)|tag_1.code()) , m_nScoreIndex , amount , round ) ;
    ...
    if (length_1 <= 2)
      nReturn += m_weights->m_mapLastTagByWord.getOrUpdateScore(std::make_pair(word_1, tag_2), m_nScoreIndex, amount, round);

    if (index > 1) {
      if (length_1 <= 2)
        nReturn += m_weights->m_mapTagByWordAndPrevChar.getOrUpdateScore(std::make_pair(word_1_last_char_2, tag_1), m_nScoreIndex, amount, round);
      if (length_1 == 1 && index < item->size())
        nReturn += m_weights->m_mapTagOfOneCharWord.getOrUpdateScore(std::make_pair(three_char, tag_1), m_nScoreIndex, amount, round);
    }
  ...
  if (index > 0)
    nReturn += m_weights->m_mapTag0Tag1Size1.getOrUpdateScore(std::make_pair(tag_0_tag_1, length_1), m_nScoreIndex, amount, round);
  if (index > 0)
    nReturn += m_weights->m_mapTag1Tag2Size1.getOrUpdateScore(std::make_pair(tag_1_tag_2, length_1), m_nScoreIndex, amount, round);
  if (index > 0)
    nReturn += m_weights->m_mapTag0Tag1Tag2Size1.getOrUpdateScore(std::make_pair(tag_0_tag_1_tag_2, length_1), m_nScoreIndex, amount, round);

  if (length_1 <= 2)
    nReturn += m_weights->m_mapTagByLastWord.getOrUpdateScore(std::make_pair(word_1, tag_0), m_nScoreIndex, amount, round);
  ...
  if (index < item->size()) {
    if (index > 0) {
      ...
      if (length_1 <= 2)
        nReturn += m_weights->m_mapTagByWordAndNextChar.getOrUpdateScore(std::make_pair(word_1_first_char_0, tag_1), m_nScoreIndex, amount, round);
...
//      nReturn += m_weights->m_mapSepCharAndNextChar.getOrUpdateScore( find_or_replace_word_cache(start_0, start_0==sentence->size()-1?start_0:start_0+1) , m_nScoreIndex , amount , round ) ; 
    ...
    if (index > 0)
      nReturn += m_weights->m_mapFirstCharBy3Tags.getOrUpdateScore(std::make_pair(first_char_0, tag_0_tag_1_tag_2), m_nScoreIndex, amount, round);
    ...
    if (index > 0)
      nReturn += m_weights->m_mapTagWordTag.getOrUpdateScore(std::make_pair(word_1, tag_0_tag_2), m_nScoreIndex, amount, round);
    if (index > 1)
      nReturn += m_weights->m_mapWordTagTag.getOrUpdateScore(std::make_pair(word_2, tag_0_tag_1), m_nScoreIndex, amount, round);
  }
...
SCORE_TYPE CTagger::getOrUpdateAppendScore(const CStringVector *sentence, const CSubStateItem *item, unsigned long index, unsigned long char_index,
    SCORE_TYPE amount, unsigned long round) {
  static SCORE_TYPE nReturn;
  assert(char_index > 0);
...
//           !m_weights->m_Knowledge->isFWorCD(chars[chars.size()-1])))
    ...
    for (j = total_size + 1; j < total_size + local_size; ++j)
      if (!m_weights->m_rules.canSeparate(j))
        bNoSep = true;
    if (!bNoSep)
    ...
    if (index > 0) {
      ...
      for (j = 0; j < m_Agenda.generatorSize(); ++j) {
        ...
        if ((m_weights->m_rules.canAppend(index)) && // ( index > 0 ) &&
            (pGenerator->getWordStart(pGenerator->size() - 1) < last_nosep || pGenerator->getTag(pGenerator->size() - 1).code() == PENN_TAG_CD
                || pGenerator->getWordLength(pGenerator->size() - 1) < m_weights->m_maxLengthByTag[pGenerator->getTag(pGenerator->size() - 1).code()])) {
          ...
          if (index + 1 == length)
            tempState.score += getOrUpdateSeparateScore(sentence, &tempState, tempState.size());
          m_Agenda.pushCandidate(&tempState);
        } // if
        pGenerator = m_Agenda.generatorNext();  // next generator
      }
