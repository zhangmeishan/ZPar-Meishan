...
void auto_train(const std::string &sOutputFile, const std::string &sFeatureFile, const std::string &sStackingPath) {

   std::cout << "Training iteration is started..." << std::endl ; std::cout.flush();

   CCharTagger parser(sFeatureFile, true);
...
int main(int argc, char* argv[]) {
   ...
   try {
      ...
      for (int i=0; i<training_rounds; ++i) 
         auto_train(options.args[1], options.args[2], sStackingPath);
