...
public:
  ...
  void GetBestTransitions(const CDependencyParse &tree, const std::vector<CDependencyLabel>&m_lCacheLabel, std::set<int> &validSet) const
  {
      validSet.clear();
      if(m_nStackSize == 0)
      {
        assert( m_nNextWord == 0 );
        int bestaction = action::encodeAction(action::SHIFT, CTag(tree[m_nNextWord].tag).code());
        validSet.insert(bestaction);
        return;
      }
      int sentLength = static_cast<int>(tree.size());
      if(m_nStackSize == 1)
      {
          int bestaction = 0;
          if(m_nNextWord == sentLength)
          {
              bestaction = action::encodeAction(action::POP_ROOT);
          }
          else
          {
              bestaction = action::encodeAction(action::SHIFT, CTag(tree[m_nNextWord].tag).code());
          }
          validSet.insert(bestaction);
          return;
      }
      else
      {
          ...
          int top1 = m_Stack[m_nStackSize-2];
          //int shifterror = 0;
          //int alerror = 0;
          //int arerror = 0;
          int bestaction = 0;
          int top0RightChildNum = 0;
          for(int i = m_nNextWord; i < sentLength; i++)
          {
              if(tree[i].head == top0)
              {
                  top0RightChildNum++;
          ...
          if(tree[top0].head == top1 && top0RightChildNum == 0)
          {
              ...
              bestaction=action::encodeAction(action::ARC_RIGHT, m_lCacheLabel[top0].code());
          }
          else if(tree[top1].head == top0 && top0 == m_nNextWord - 1)
          {
              ...
              bestaction=action::encodeAction(action::ARC_LEFT, m_lCacheLabel[top1].code());
          }
          else if(m_nNextWord < sentLength)
          {
              bestaction = action::encodeAction(action::SHIFT, CTag(tree[m_nNextWord].tag).code());
          }
          else
...
//         }
   ...
   friend std::ostream & operator << (std::ostream& out, const CStateItem & item) {
     out << action::getUnlabeledAction(item.m_nLastAction)
         << "-" << action::getLabel(item.m_nLastAction)
         << " (" << item.score
         << "): (";
     for (int i = 0; i < item.m_nStackSize; ++ i) {
       out << item.m_Stack[i];
       if (i + 1 < item.m_nStackSize) { out << "|"; }
     }
     out << "): (";
     for (int i = 0; i < item.m_nActionSize; ++ i) {
       out << item.m_Action[i];
       if (i + 1 < item.m_nActionSize) { out << "|"; }
     }
     out << ") ";
     out << item.m_nNextWord << " " << item.m_lCache->size() << std::endl;
     return out;
   }
