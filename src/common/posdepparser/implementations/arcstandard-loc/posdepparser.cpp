...
inline void CPosDepParser::getOrUpdateStackScore( const CStateItem *item, CPackedScoreType<SCORE_TYPE, action::MAX> &retval, const unsigned &action, SCORE_TYPE amount , int round ) {
   ...
   if(n0_index != -1 && (action == action::NO_ACTION || action::getUnlabeledAction(action) == action::SHIFT))
   {
	   ...
	   if ( p1_index != -1 ) {
	      ...
	      if ( last_length <= 2 )cast_weights->m_mapLastWord.getOrUpdateScore( retval, p1_word, action, m_nScoreIndex, amount, round);
	      if ( last_length <= 2 )cast_weights->m_mapLastTagCurrentWord.getOrUpdateScore( retval, std::make_pair(p1_word, p1_tag), action, m_nScoreIndex, amount, round);
	      if ( length <= 2 )
	      {
	   ...
	   if ( n1_index != -1 ) {
	      if ( next_length <= 2 )cast_weights->m_mapNextWord.getOrUpdateScore( retval, n1_word, action, m_nScoreIndex, amount, round);
	      if ( length <= 2 )
	      {
...
void CPosDepParser::work( const bool bTrain , const CStringVector &sentence , CDependencyParse *retval , const CDependencyParse &correct, const CSTackFeatsVec  & stackfeats , int nBest , SCORE_TYPE *scores ) {
   ...
   static CStateItem predBestState(&m_lCache) ;
   static CPackedScoreType<SCORE_TYPE, action::MAX> packed_scores;

   std::set<int> valid_actions;
  ...
  // }
   ...
   for (index=0; index<length*2; ++index) {
      ...
      // iterate generators
      assert(m_Agenda->generatorSize() == 1);
      for (int j=0; j<m_Agenda->generatorSize(); ++j) {
		 ...
		 if (bTrain)
		 {
		     pGenerator->GetBestTransitions(correct, m_lCacheLabel, valid_actions);

		     if(CORRECT_PERC == 100)
		     {
                 assert(valid_actions.size() == 1);
                 correctState = *pGenerator;

                 correctState.StandardMoveStep(correct, m_lCacheLabel);


                for (std::set<int>::iterator it=valid_actions.begin(); it!=valid_actions.end(); ++it)
                 {
                     //std::cout << ' ' << *it;
                    if(*it != correctState.m_nLastAction)
                    {
                        std::cout << "gold: " << correctState.m_nLastAction << "\tpred: " << "" << *it << std::endl;
                    }
                 }
                 //std::cout << "==================g" << std::endl;
                 std::cout.flush();
         ...
         for (unsigned i=0; i<m_Beam->size(); ++i) {
            ...
            if (bTrain)
            {
                //std::cout << ' ' << m_Beam->item(i)->action;
                if(valid_actions.find(m_Beam->item(i)->action) != valid_actions.end()
                        && (pCandidate.score > correctState.score || correctState.m_nActionSize == 0)
                        )
                {
                    correctState = pCandidate;
                }

                if(pCandidate.score > predBestState.score || predBestState.m_nActionSize == 0)
                {
                    predBestState = pCandidate;
                }
            }
         }
         ...
         //{
             //std::cout << "==================p" << std::endl;
             //if(correctState.m_nActionSize == 0)
            // {
            //     std::cout << "error........" << std::endl;
            // }
         //}
      ...
      if (bTrain) {
          ...
          if(*bestitem != predBestState)
          {
              std::cout << "error" << std::endl;
          }
          ++m_nTrainingRound; // each training round is one transition-action
          if(predBestState != correctState)
          {
              updateScoresForStates(&predBestState, &correctState, 1, -1) ;
          }

            srand(m_nTrainingRound);

            m_Agenda->clearCandidates();

            if(rand()%100 < CORRECT_PERC)
            {
                m_Agenda->pushCandidate(&correctState);
            }
            else
            {
                m_Agenda->pushCandidate(&predBestState);
            }
