...
inline void CPosDepParser::getOrUpdateStackScore( const CStateItem *item, CPackedScoreType<SCORE_TYPE, action::MAX> &retval, const unsigned &action, SCORE_TYPE amount , int round ) {
   ...
   if(m_stackfeats.size() == m_lCache.size() )
   {
	   if(st0_index != -1)
	   {
		   ...
		   const int stackheadst0_1 = m_stackfeats[st0_index].m_feats.size() >= 2 ? atoi(m_stackfeats[st0_index].m_feats[0].c_str()) -1 : -2;
		   const int stackheadst1_1 = st1_index != -1 && m_stackfeats[st1_index].m_feats.size() >= 2 ? atoi(m_stackfeats[st1_index].m_feats[0].c_str()) -1 : -2;
		   ...
		   const int stackheadst0_2 = m_stackfeats[st0_index].m_feats.size() >= 4 ? atoi(m_stackfeats[st0_index].m_feats[2].c_str()) -1 : -2;
		   const int stackheadst1_2 = st1_index != -1 && m_stackfeats[st1_index].m_feats.size() >= 4 ? atoi(m_stackfeats[st1_index].m_feats[2].c_str()) -1 : -2;
		   // arcleft, 2; arcright, 3; st0root, 4; other 1
		   static int arctype2;
...
*/
   ...
   if(n0_index != -1 && (action == action::NO_ACTION || action::getUnlabeledAction(action) == action::SHIFT))
   {
	   ...
	   if(m_stackfeats.size() == m_lCache.size())
	   {
		   const CTag &stacktag1 = m_stackfeats[n0_index].m_feats.size() >= 2 ? CTag(m_stackfeats[n0_index].m_feats[1]) : g_noneTag;
		   ...
		   const CTag &stacktag2 = m_stackfeats[n0_index].m_feats.size() >= 4 ? CTag(m_stackfeats[n0_index].m_feats[3]) : g_noneTag;
