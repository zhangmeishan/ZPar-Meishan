...
#ifdef MULTI_SCHEMA
   virtual void parse( const CTwoStringVector &sentence , CDependencyParse *retval0 , CDependencyParse *retval1 , int nBest=1, depparser::SCORE_TYPE *scores=0 ) = 0 ;
#else
#ifdef JOINT_MORPH
...
#ifdef MULTI_SCHEMA
   virtual void train( const CDependencyParse &correct0 ,const CDependencyParse &correct1 , int round ) = 0 ;

   virtual void parse_conll( const CCoNLLInput &sentence , CCoNLLOutput *retval0 , CCoNLLOutput *retval1 , int nBest=1, depparser::SCORE_TYPE *scores=0 ) {
      THROW("depparser_base.h: the method parse_conll is not implemented");
   }
   virtual void train_conll( const CCoNLLOutput &correct0 , const CCoNLLOutput &correct1 ,int round ) {
