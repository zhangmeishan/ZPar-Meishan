...
#define refer_or_allocate_tuple4(x, o1, o2, o3, o4) do { \
  if (amount == 0) { \
    x.refer(o1, o2, o3, o4); \
  } else { \
    x.allocate(o1, o2, o3, o4); \
  } \
} while (0);
...
CDepParser::GetOrUpdateStackScore(const CStateItem * item,
  ...
  const int curIndex = item->nextactionindex();
...
#define __GET_OR_UPDATE_SCORE(temp, feature) \
  ...
  CTuple4<CWord, CWord, CTag, CTag>     wwtt;
  ...
  if (-1 != S0id && -1 != S1id) {
    ...
    //refer_or_allocate_tuple3(wtt, &S0wt.word, &S1wt.tag, &S0wt.tag);
    //__GET_OR_UPDATE_SCORE(S0wtS1t, wtt);        //  new

    //refer_or_allocate_tuple3(wtt, &S1wt.word, &S0wt.tag, &S1wt.tag);
    //__GET_OR_UPDATE_SCORE(S0tS1wt, wtt);        //  new

    //refer_or_allocate_tuple4(wwtt, &S0wt.word, &S1wt.word, &S0wt.tag,  &S1wt.tag);
    //__GET_OR_UPDATE_SCORE(S0wtS1wt, wwtt);        //  new
  }
  ...
  if (-1 != S0id && -1 != N0id) {
    ...
    //refer_or_allocate_tuple3(wtt, &S0wt.word, &S0wt.tag,  &N0wt.tag);
    //__GET_OR_UPDATE_SCORE(S0wtN0t, wtt);        //  new

    //refer_or_allocate_tuple3(wtt, &N0wt.word, &S0wt.tag,  &N0wt.tag);
    //__GET_OR_UPDATE_SCORE(S0tN0wt, wtt);        //  new
  }

  //if (-1 != S1id && -1 != N0id) {
  //    refer_or_allocate_tuple3(ttt, &S1wt.tag, &S0wt.tag,  &N0wt.tag);
  //    __GET_OR_UPDATE_SCORE(S1tS0tN0t, ttt);    // new
  //}

  //if (-1 != S0id && -1 != N1id) {
  //    refer_or_allocate_tuple3(ttt, &N1wt.tag, &S0wt.tag,  &N0wt.tag);
  //    __GET_OR_UPDATE_SCORE(S0tN0tN1t, ttt);   //new
  //}
...
//    __GET_OR_UPDATE_SCORE(S0wS1wDist, wwi);

//    refer_or_allocate_tuple3(tti, &S0wt.tag, &S1wt.tag, &dist);
//    __GET_OR_UPDATE_SCORE(S0tS1tDist, tti);
  ...
  if (-1 != S0id && -1 != S1id) {
      ...
      int dist = encodeLinkDistance(S1id, S0id);
      int S0idGuide = guideddirection;
      if(S0id < item->size_another() && item->label_another(S0id) != CDependencyLabel::NONE)
      {
          S0idGuide = item->label_another(S0id) * 10 + guideddirection;
      }

      int S1idGuide = guideddirection;

      if(S1id < item->size_another() && item->label_another(S1id) != CDependencyLabel::NONE)
      {
          S1idGuide = item->label_another(S1id) * 10 + guideddirection;
      }
      ...
      //if(guideddirection > -1)
      {
          ...
          __GET_OR_UPDATE_SCORE(DistGuide,    guideddirection*10 +dist);   //104-a

          __GET_OR_UPDATE_SCORE(Guide,    guideddirection);   //104


          refer_or_allocate_tuple2(wi, &S0wt.word, &S0idGuide);
          __GET_OR_UPDATE_SCORE(S0wS0LGuide, wi);  //105

          refer_or_allocate_tuple2(ti, &S0wt.tag, &S0idGuide);
           __GET_OR_UPDATE_SCORE(S0tS0LGuide, ti);  //106

          refer_or_allocate_tuple2(wi, &S1wt.word, &S0idGuide);
          __GET_OR_UPDATE_SCORE(S1wS0LGuide, wi);  //107

          refer_or_allocate_tuple2(ti, &S1wt.tag, &S0idGuide);
          __GET_OR_UPDATE_SCORE(S1tS0LGuide, ti);  //108

          __GET_OR_UPDATE_SCORE(S0LGuide,    S0idGuide);  //109

          __GET_OR_UPDATE_SCORE(DistS0LGuide,    S0idGuide*10 +dist);  //109 -a




          refer_or_allocate_tuple2(wi, &S0wt.word, &S1idGuide);
          __GET_OR_UPDATE_SCORE(S0wS1LGuide, wi);  //110

          refer_or_allocate_tuple2(ti, &S0wt.tag, &S1idGuide);
          __GET_OR_UPDATE_SCORE(S0tS1LGuide, ti);  //111

          refer_or_allocate_tuple2(wi, &S1wt.word, &S1idGuide);
          __GET_OR_UPDATE_SCORE(S1wS1LGuide, wi);  //112

          refer_or_allocate_tuple2(ti, &S1wt.tag, &S1idGuide);
          __GET_OR_UPDATE_SCORE(S1tS1LGuide, ti);  //113

          __GET_OR_UPDATE_SCORE(S1LGuide,    S1idGuide);  //114

          __GET_OR_UPDATE_SCORE(DistS1LGuide,    S1idGuide*10 +dist);  //114-a

          __GET_OR_UPDATE_SCORE(S0LGuideS1LGuide,    S1idGuide*10000+S0idGuide);  //115

          refer_or_allocate_tuple3(tti, &S0wt.tag, &S1wt.tag, &guideddirection);
          __GET_OR_UPDATE_SCORE(S0tS1tGuide, tti);  //116

          refer_or_allocate_tuple3(tti, &S0wt.tag, &S1wt.tag, &S0idGuide);
          __GET_OR_UPDATE_SCORE(S0tLGuideS1t, tti);  //117

          refer_or_allocate_tuple3(tti, &S0wt.tag, &S1wt.tag, &S1idGuide);
          __GET_OR_UPDATE_SCORE(S0tS1tLGuide, tti);  //118
...
CDepParser::UpdateScoresForStates(const CStateItem * predicated_state,
  ...
  const CStateItem * predicated_state_chain[kMaxSentenceSize * 4+1];
  const CStateItem * correct_state_chain[kMaxSentenceSize * 4+1];
  ...
  for (const CStateItem * p = predicated_state; p; p = p->previous_) {
    ...
    //if( (num_predicated_states == (predicated_state->m_nActionNum[0] + predicated_state->m_nActionNum[1])) && p->previous_ )
    //{
    //    std::cout << "Number of predicted action :" << num_predicated_states << std::endl;
    //    std::cout << *p << std::endl;
    //    std::cout << *(p->previous_) << std::endl;
    //}
  }
  ...
  for (const CStateItem * p = correct_state; p; p = p->previous_) {
    ...
    ++ num_correct_states;
    //if( (num_correct_states == (correct_state->m_nActionNum[0] + correct_state->m_nActionNum[1])) && p->previous_ )
    //{
    //    std::cout << "Number of correct action :" << num_correct_states << std::endl;
    //    std::cout << *p << std::endl;
    //    std::cout << *(p->previous_) << std::endl;
    //}
  }

  if(num_correct_states != num_predicated_states)
  {
      std::cout << "Number of predicated action don't equals the correct one" << std::endl;
      std::cout << "Number of predicted action :" << num_predicated_states << std::endl;
      std::cout << *predicated_state << std::endl;
      std::cout << *(predicated_state_chain[num_predicated_states-1]) << std::endl;
      std::cout << "Number of correct action :" << num_correct_states << std::endl;
      std::cout << *correct_state << std::endl;
      std::cout << *(correct_state_chain[num_correct_states-1]) << std::endl;
  }
...
void CDepParser::train(const CDependencyParse &correct0 ,const CDependencyParse &correct1 , int round) {
   ...
   assert(!m_bCoNLL);
   assert(IsProjectiveDependencyTree(correct0) && IsProjectiveDependencyTree(correct1) );
   /*{
       std::cout << "nonprojective tree" << std::endl;
       ++m_nTrainingRound;
       return;
   }*/
