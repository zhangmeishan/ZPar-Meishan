...
#ifdef LABELED
   std::vector< CDependencyLabel > m_lCacheLabel0;
   std::vector< CDependencyLabel > m_lCacheLabel1;
#endif
...
public:
   void parse( const CTwoStringVector &sentence , CDependencyParse *retval0 , CDependencyParse *retval1 , int nBest=1 , depparser::SCORE_TYPE *scores=0 ) ;
   void train( const CDependencyParse &correct0 , const CDependencyParse &correct1 , int round ) ;
   void extract_features( const CDependencyParse &input0 , const CDependencyParse &input1) ;

   void parse_conll( const CCoNLLInput &sentence , CCoNLLOutput *retval0 ,  CCoNLLOutput *retval1 , int nBest=1, depparser::SCORE_TYPE *scores=0 ) ;
   void train_conll( const CCoNLLOutput &correct0 , const CCoNLLOutput &correct1 , int round ) ;
   void extract_features_conll( const CCoNLLOutput &input0, const CCoNLLOutput &input1 ) ;
...
private:
   ...
   void work( const bool bTrain, const CTwoStringVector &sentence , CDependencyParse *retval0, CDependencyParse *retval1,
           const CDependencyParse &correct0, const CDependencyParse &correct1, int nBest, depparser::SCORE_TYPE *scores ) ;
