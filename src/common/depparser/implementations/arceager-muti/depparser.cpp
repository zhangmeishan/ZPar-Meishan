...
inline void CDepParser::getOrUpdateStackScore( const CStateItem *item, CPackedScoreType<SCORE_TYPE, action::MAX> &retval, const unsigned &action, SCORE_TYPE amount , int round ) {
   ...
   // stacked features
    if (-1 != st_index && -1 != n0_index) {
        int guideddirection = -1;
        if(st_index < item->size_another() && item->head_another(st_index) == n0_index)
        {
            guideddirection = 0;
        }

        if(n0_index < item->size_another() && item->head_another(n0_index) == st_index)
        {
            guideddirection = 1;
        ...
        {
            refer_or_allocate_tuple2(word_int, &st_word, &guideddirection);
            cast_weights->S0wGuide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //100

            refer_or_allocate_tuple2(tag_int, &st_tag, &guideddirection);
            cast_weights->S0tGuide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ; //101

            refer_or_allocate_tuple2(word_int, &n0_word, &guideddirection);
            cast_weights->N0wGuide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //102

            refer_or_allocate_tuple2(tag_int, &n0_tag, &guideddirection);
            cast_weights->N0tGuide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ; //103

            cast_weights->Guide.getOrUpdateScore(retval,  guideddirection, action, m_nScoreIndex, amount, round ) ;   //104

            if(st_index < item->size_another() && item->label_another(st_index) != CDependencyLabel::NONE)
            {
                const int & st_indexGuide = item->label_another(st_index);

                refer_or_allocate_tuple2(word_int, &st_word, &st_indexGuide);
                cast_weights->S0wS0Guide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //105

                refer_or_allocate_tuple2(tag_int, &st_tag, &st_indexGuide);
                cast_weights->S0tS0Guide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ; //106

                refer_or_allocate_tuple2(word_int, &n0_word, &st_indexGuide);
                cast_weights->N0wS0Guide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //107

                refer_or_allocate_tuple2(tag_int, &n0_tag, &st_indexGuide);
                cast_weights->N0tS0Guide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ;   //108

                cast_weights->S0Guide.getOrUpdateScore(retval,  st_indexGuide, action, m_nScoreIndex, amount, round ) ;  //109
            }


            if(n0_index < item->size_another() && item->label_another(n0_index) != CDependencyLabel::NONE)
            {
                const int & n0_indexGuide = item->label_another(n0_index);
                refer_or_allocate_tuple2(word_int, &st_word, &n0_indexGuide);
                cast_weights->S0wN0Guide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //110

                refer_or_allocate_tuple2(tag_int, &st_tag, &n0_indexGuide);
                cast_weights->S0tN0Guide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ; //111

                refer_or_allocate_tuple2(word_int, &n0_word, &n0_indexGuide);
                cast_weights->N0wN0Guide.getOrUpdateScore(retval, word_int, action, m_nScoreIndex, amount, round ) ; //112

                refer_or_allocate_tuple2(tag_int, &n0_tag, &n0_indexGuide);
                cast_weights->N0tN0Guide.getOrUpdateScore(retval, tag_int, action, m_nScoreIndex, amount, round ) ; //113

                cast_weights->N0Guide.getOrUpdateScore(retval,  n0_indexGuide, action, m_nScoreIndex, amount, round ) ;  //114
            }
...
inline void CDepParser::reduce( const CStateItem *item, const CPackedScoreType<SCORE_TYPE, action::MAX> &scores ) {
   ...
   if(index == 0)
   {
       scoredaction.action = action::REDUCE0;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
   else
   {
       scoredaction.action = action::REDUCE1;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
}
...
inline void CDepParser::shift( const CStateItem *item, const CPackedScoreType<SCORE_TYPE, action::MAX> &scores ) {
   ...
   if(index == 0)
   {
       scoredaction.action = action::SHIFT0;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
   else
   {
       scoredaction.action = action::SHIFT1;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
}
...
inline void CDepParser::poproot( const CStateItem *item, const CPackedScoreType<SCORE_TYPE, action::MAX> &scores ) {
   ...
   if(index == 0)
   {
       scoredaction.action = action::POP_ROOT0;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
   else
   {
       scoredaction.action = action::POP_ROOT1;
       scoredaction.score = item->score + scores[scoredaction.action];
       m_Beam->insertItem(&scoredaction);
   }
}
...
void CDepParser::work( const bool bTrain , const CTwoStringVector &sentence , CDependencyParse *retval0 , CDependencyParse *retval1 ,
        const CDependencyParse &correct0 , const CDependencyParse &correct1 , int nBest , SCORE_TYPE *scores ) {
   ...
   for ( index=0; index<length; ++index ) {
      ...
      if (bTrain && m_weights->rules()) {
         // the root
         if ( correct0[index].head == DEPENDENCY_LINK_NO_HEAD && canBeRoot(m_lCache[index].tag.code())==false) {
            TRACE("Rule contradiction: " << m_lCache[index].tag.code() << " can be root.");
            bContradictsRules = true;
         }
         // head left
         if ( correct0[index].head < index && hasLeftHead(m_lCache[index].tag.code())==false) {
            TRACE("Rule contradiction: " << m_lCache[index].tag.code() << " has left head.");
            bContradictsRules = true;
         }
         // head right
         if ( correct0[index].head > index && hasRightHead(m_lCache[index].tag.code())==false) {
   ...
   if (bTrain) {
      for (index=0; index<length; ++index) {
         m_lCacheLabel0.push_back(CDependencyLabel(correct0[index].label));
         m_lCacheLabel1.push_back(CDependencyLabel(correct1[index].label));
         //if (m_weights->rules() && !canAssignLabel(m_lCache, correct0[index].head, index, m_lCacheLabel0[index])) {
         //   TRACE("Rule contradiction: " << correct[index].label << " on link head " << m_lCache[correct0[index].head].tag.code() << " dep " << m_lCache[index].tag.code());
         //   bContradictsRules = true;
        // }
      }
   }
      ...
      if (bTrain) {
         ...
         if (!bCorrect) {
            //TRACE("Error at the "<<correctState.size()<<"th word; total is "<<correct.size())
             int curIndex = correctState.nextactionindex();
            TRACE("ERROR at the " << correctState.size() << "th word for schema " << curIndex);
         ...
         {
#ifdef LABELED
            correctState.StandardMoveStep(correct0, correct1, m_lCacheLabel0, m_lCacheLabel1);
#else
            correctState.StandardMoveStep(correct0, correct1);
#endif
         }
   ...
   for (int i=0; i<std::min(m_Agenda->generatorSize(), nBest); ++i) {
      ...
      if (pGenerator) {
         pGenerator->GenerateTree( sentence , retval0[i] , retval1[i] ) ;
...
void CDepParser::parse( const CTwoStringVector &sentence , CDependencyParse *retval0 , CDependencyParse *retval1 , int nBest , SCORE_TYPE *scores ) {
...
#endif
   work( true , sentence , &outout0 , &outout1 , correct0 , correct1 , 1 , 0 ) ;
...
void CDepParser::extract_features(const CDependencyParse &input0, const CDependencyParse &input1) {
   ...
   for (int i=0; i<input0.size(); ++i) {
      m_lCache.push_back(CTaggedWord<CTag, TAG_SEPARATOR>(input0[i].word, input0[i].tag));
#ifdef LABELED
...
#ifdef LABELED
   item.StandardMoveStep(input0, input1, m_lCacheLabel0, m_lCacheLabel1);
#else
   item.StandardMoveStep(input0, input1);
#endif
   }
...
void CDepParser::parse_conll( const CCoNLLInput &sentence , CCoNLLOutput *retval0 , CCoNLLOutput *retval1 , int nBest, SCORE_TYPE *scores ) {
   ...
   work(false, input, outout0, outout1, empty, empty, nBest, scores ) ;
...
void CDepParser::train_conll( const CCoNLLOutput &correct0 , const CCoNLLOutput &correct1 , int round ) {
...
void CDepParser::extract_features_conll( const CCoNLLOutput &input0, const CCoNLLOutput &input1) {
