...
void auto_train(const std::string &sOutputFile, const std::string &sFeatureFile, const bool &bRules, const std::string &sSuperPath, const bool &bCoNLL, const bool &bExtract, const std::string &sMetaPath) {
...
#ifdef MULTI_SCHEMA
   CDependencyParse ref_sent1;
   CCoNLLOutput ref_conll1;
#endif
   ...
   while( bCoNLL ? is>>ref_conll : is>>ref_sent ) {
...
#ifdef MULTI_SCHEMA
       if(bCoNLL ? is>>ref_conll1 : is>>ref_sent1)
       {
       }
       else
       {
           ASSERT(false, "Internal error: the second schema is not able to be readed out.");
       }
#endif
      ...
      if (bExtract) {
         ...
         {
#ifdef MULTI_SCHEMA
            parser.extract_features_conll( ref_conll, ref_conll1 );
#else
         ...
         {
#ifdef MULTI_SCHEMA
            parser.extract_features( ref_sent, ref_sent1 );
#else
         ...
         {
#ifdef MULTI_SCHEMA
            parser.train_conll( ref_conll, ref_conll1, nCount );
#else
         ...
         {
#ifdef MULTI_SCHEMA
            parser.train( ref_sent, ref_sent1, nCount );
#else
