...
public:
   const static unsigned long MAX=(1L<<(CDisConstituent::SIZE+1+1+CActionType::SIZE));

protected:
   // CONSTITUENT(CDisConstituent::SIZE) TEMP(1 bit) HEAD(1) ACTIONTYPE(CActionType::SIZE)
   ...
   const static unsigned long CONSTITUENT_MASK = ((1L<<CDisConstituent::SIZE)-1)<<CONSTITUENT_SHIFT;
...
public:
  CAction() : action(0) {}
  CAction(unsigned long code ) : action(code) {}
...
public:
   inline void encodeReduce(const unsigned long &constituent, bool single_child, bool head_left, bool temporary) {
      ...
      action = ((CDisConstituent::encodeTmp(constituent, temporary))<<CONSTITUENT_SHIFT |
   ...
   inline void encodeShift(const unsigned long &constituent) {
...
public:
   inline std::string str() const {
      ...
      retval += " "; retval += CDisConstituent(getConstituent()).str();
      return retval;
   }
