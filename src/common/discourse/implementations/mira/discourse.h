...
private:
   ...
   discourse::CWeight *m_gold;
   discourse::CWeight *m_delta[discourse::MIRA_SIZE];
   discourse::SCORE_TYPE m_dist[discourse::MIRA_SIZE];
   discourse::SCORE_TYPE m_alpha[discourse::MIRA_SIZE];
...
public:
   ...
   CDisParser( const std::string &sFeatureDBPath , bool bTrain ) : CDisParserBase(sFeatureDBPath, bTrain),
      ...
      if (bTrain) {
         m_gold = new discourse::CWeight( bTrain, 512 );
         for (unsigned i=0; i<discourse::MIRA_SIZE; ++i) {
            m_delta[i] = new discourse::CWeight( bTrain, 512 );
         }
      }
      else {
          m_gold = 0;
...
private:
   ...
   int computeLoss(const discourse::CStateItem *outout , const discourse::CStateItem *correct);

   void updateScoresForMultipleStates( const discourse::CStateItem *outout_start , const discourse::CStateItem *output_end , const discourse::CStateItem *correct ) ;
