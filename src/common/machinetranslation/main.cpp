...
/****************************************************************
 *                                                              *
 * main.cpp - main app of the general translator.               *
 *                                                              *
...
void process(const std::string sInputFile, const std::string sOutputFile, const std::string sFeatureFile, const std::string &sTgtLMPath, const std::string &sOsmLMPath, const std::string &sPrLMPath, unsigned long nBest, const bool bScores, const std::string &sStackingPath) {

   std::cout << "Parsing started" << std::endl;

   int time_start = clock();

   CMachineTrans mt(sFeatureFile, false) ;

   // load target language model
   mt.init_tagetlm(sTgtLMPath);

   // load operation language model
   mt.init_osmlm(sOsmLMPath);

   // load prior probability model
   mt.init_priorlm(sPrLMPath);


   CSentenceReader *input_reader;
   std::ofstream os(sOutputFile.c_str());
   std::ofstream *os_scores=0;
   machinetranslation::SCORE_TYPE *scores=0;
   assert(os.is_open());
   CStringVector input_sent;
   CAlignedSentencePair *outout_sent;
   ...
   if (bScores) {
      scores = new machinetranslation::SCORE_TYPE[nBest];
      os_scores = new std::ofstream(std::string(sOutputFile+".scores").c_str());
   }


   outout_sent = new CAlignedSentencePair[nBest];
   ...
   while( bReadSuccessful ) {
      ...
      // check size
      if ( input_sent.size() > machinetranslation::MAX_SENTENCE_SIZE ) {
         ...
         for (unsigned i=0; i<nBest; ++i) {
    	  ...
    	  mt.translator( input_sent , outout_sent , stackfeats, nBest , scores ) ;
      }
      
      // Ouptut sent
      for (unsigned i=0; i<nBest; ++i) {
         os << outout_sent[i] << std::endl;
...
//      else
      process(options.args[1], options.args[2], options.args[3], options.args[4], options.args[5], options.args[6], nBest, bScores, sStackingPath);
      return 0;
