...
public:
   ...
   enum CODE {NO_ACTION=0, SHIFT_Y=1, SHIFT_X=2, INSERT_GAP=3,
	   JUMP_BACK=4, JUMP_FORWARD=5, FINISH=6, IDLE=7,
	   GENERATE_XX=8, GENERATE_XY=9, GENERATE_X=10, GENERATE_Y=11};
...
public:
   ...
   std::string str()
   {
	   ...
	   case CActionType::GENERATE_X:
	      os = "GENERATE_X";
	      break;
	   case CActionType::GENERATE_Y:
	      os = "GENERATE_Y";
	      break;
	   case CActionType::GENERATE_XX:
	       os = "GENERATE_XX";
	       break;
...
inline std::istream & operator >> (std::istream &is, CActionType &action) {
   ...
   if (s=="NO_ACTION")
      ...
      action.code = CActionType::SHIFT_Y;
   else if (s=="GENERATE_XY")
      action.code = CActionType::GENERATE_XY;
   else if (s=="GENERATE_X")
      action.code = CActionType::GENERATE_X;
   else if (s=="GENERATE_Y")
      action.code = CActionType::GENERATE_Y;
   else if (s=="GENERATE_XX")
      action.code = CActionType::GENERATE_XX;
   else if (s=="SHIFT_X")
...
public:
	...
	{
		MAX = rulesize + CActionType::GENERATE_XX;
	}
...
public:
   ...
   inline bool isShiftY(const std::vector<int >& mapTranslateRuleTypes) const { return type(mapTranslateRuleTypes)==CActionType::SHIFT_Y; }
   ...
   inline bool isGenerateXX(const std::vector<int >& mapTranslateRuleTypes) const { return type(mapTranslateRuleTypes)==CActionType::GENERATE_XX; }
   ...
   inline bool isGenerateY(const std::vector<int >& mapTranslateRuleTypes) const { return type(mapTranslateRuleTypes)==CActionType::GENERATE_Y; }
...
public:
   ...
   static unsigned long encodeShiftY() { return CActionType::SHIFT_Y; }
   static unsigned long encodeShiftX() { return CActionType::SHIFT_X; }
   static unsigned long encodeGenerateXX() { return CActionType::GENERATE_XX; }
   static unsigned long encodeGenerateXY(int ruleId) { return CActionType::GENERATE_XX+1+ruleId; }
   static unsigned long encodeGenerateX(int ruleId) {  return CActionType::GENERATE_XX+1+ruleId; }
   static unsigned long encodeGenerateY(int ruleId)  {  return CActionType::GENERATE_XX+1+ruleId; }

public:
   inline unsigned long type(const std::vector<int >& mapTranslateRuleTypes) const {
      if(action < CActionType::GENERATE_XX+1)
      {
    	  return action;
      }
      else
      {
    	  int ruleId = action - CActionType::GENERATE_XX -1;
    	  //if(ruleId == 10146)
    	  //{
    		//  std::cout << "debug:" << mapTranslateRuleTypes.size() << std::endl;
    		 // std::cout.flush();
    	  //}
    	  if(ruleId == mapTranslateRuleTypes.size() || mapTranslateRuleTypes[ruleId] == -1)
    	  {
    		  return CActionType::GENERATE_Y;
    	  }
    	  else if(mapTranslateRuleTypes[ruleId] == 1)
    	  {
    		  return CActionType::GENERATE_X;
    	  }
    	  else
    	  {
    		  assert(mapTranslateRuleTypes[ruleId] == 0);
   ...
   int getRuleID()
   {
	      ...
	      {
	    	  return action - CActionType::GENERATE_XX -1;
	      }
   }
