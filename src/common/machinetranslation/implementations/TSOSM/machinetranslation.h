...
public:
   ...
   m_targetLM4(1<<20), m_osmLM4(1<<20), m_priorLM4(1<<20){
      ...
      if (bTrain)m_delta_weight = new machinetranslation :: CWeight("", bTrain, 512 );
...
private:
   ...
   inline void AddOneRule(const std::string& condition_str, const std::vector<CWord>& translated_words, float weight1, float weight2, float weight3, int ruletype);

   inline int FindOneRule(const std::string& condition_str, const std::vector<CWord>& translated_words);
