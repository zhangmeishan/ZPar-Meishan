 ...
 *
 * Translate target language (X) to source language (Y)
 *==============================================================*/

class CStateItem {
public:
   int m_StackX[MAX_SENTENCE_SIZE+1];                    // stack of target words that are currently  processed
   ...
   int m_StackXAlign[MAX_SENTENCE_SIZE+1];              // stack of align map length that are currently  processed (consecutive m v.s. random n map)
   ...
   unsigned long m_Action[MAX_ACTION_SIZE+1];
   unsigned long m_ActionType[MAX_ACTION_SIZE+1];
   int m_nActionSize;
...
public:
   ...
   bool ShiftX()
   {
	   if(m_nStackXSize >= MAX_SENTENCE_SIZE) return false;
   ...
   bool GenerateXY(const std::vector<int>& tgt_word_sequences, const std::vector<CWord>& translated_words, int rule_id)
   {
	   assert(m_nBufferXSize == 0 && tgt_word_sequences.size() > 0);
	   //m_StackY[m_nStackYSize] = (*m_lCache)[m_nStackYSize].code();
	   //m_nStackYSize++;

	   for(int i = 0; i < translated_words.size() && m_nStackYSize < MAX_SENTENCE_SIZE; i++)
	   {
		   m_StackY[m_nStackYSize] = translated_words[i].code();
		   m_nStackYSize++;
	   }
	   ...
	   {
		   m_StackX[m_nCurrentIndexInStackX] = tgt_word_sequences[0];
	   ...
	   if(tgt_word_sequences.size() > 1)
	   {
		   for(int i = tgt_word_sequences.size()-1; i > 0; i--)
		   {
			   m_BufferX[m_nBufferXSize] = tgt_word_sequences[i];
   ...
   bool GenerateX(const std::vector<int>& tgt_word_sequences, int rule_id)
   {
	   ...
	   //currently we delete one target word eanch action
	   if(tgt_word_sequences.size() > 1)
	   {
		   for(int i = tgt_word_sequences.size()-1; i > 0; i--)
		   {
   ...
   bool Finish()
   {
	   assert(m_nBufferXSize == 0 && m_nStackXSize == m_lCache->size());
...
public:
   ...
   bool Move (const unsigned long &ac, const std::vector<int> *p_tgt_word_sequences,
   const std::vector<CWord> *p_translated_words, CTranslateTypeVector& mapTranslateRuleTypes) {
	   CAction action = ac;
	   int rule_id =action.getRuleID();

	   if(m_nActionSize >= MAX_ACTION_SIZE)return false;
	   ...
	   {
	         return GenerateXY(*p_tgt_word_sequences, *p_translated_words, rule_id);
	   }
	   else if(action.isGenerateX(mapTranslateRuleTypes))
	   {
	         return GenerateX(*p_tgt_word_sequences, rule_id);
	   }
	   else
	   {
...
public:
   ...
   void GenerateTree(CAlignedSentencePair &output ) const {
	   ...
	   {
		   output.m_tgt_sent.push_back((*m_lCache)[i].str());
	   }

	   for(int i = 0; i < m_nStackYSize; i++)
	   {
		   output.m_src_sent.push_back(CWord(m_StackY[i]).str());
	   }
